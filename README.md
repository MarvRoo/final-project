...
> **Inputs of our project will include: inputting a letter (ex. a,b,c,d)  to  select a specific hint/clue (a character's testimony or an item used by the characters), and asking for the player's input to select who they think the murderer is. Each action a player makes will impact their health; these changes to their health are hidden from the player and can determine what type of ending they get. Outputs of our project will include: outputting the character's testimonies or the item's clue, outputting if the player was right or wrong from guessing who the murderer is, the number of chances they have left before they loose, and outputting who the murderer is if the player wasn't able to guess correctly.**
